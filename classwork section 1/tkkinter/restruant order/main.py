...
class RestrauntOrderManagment:
    def __init__(self, root):
        ...
        frame.place(relx= 0.5, rely=0.5,
                    anchor=tk.CENTER)
        ttk.Label(frame,
                  text='Restruant Order Management',
        ...
        for i, (item, price) in enumerate(self.menu_items.items(), start=1):
            ...
            ttk.Label(frame, text="Currency:",
                      font=("Arial, 12")).grid(row=len(self.menu_items) + 1,
                                               ...
                                               padx=10,
                                               pady=5)
            currency_dropdown = ttk.Combo(frame,
    ...
    def update_menu_prices(self, *args):
        ...
        for item, label in self in self.menu_items.items():
    ...
    def place_order(self):
        ...
        symbol = "₹" if currency == "INR" else '$'
        rate = self.exchange_rate if currency == "INR" else 1
        for item, entry in self in self.menu_items.items():
            quantity = entry.get()
            if quantity.isdigit():
                ...
                total_cost += cost
                if quantity > 0:
                    order_summery += f"{item}: {quantity} x {symbol}{price} = {symbol}{cost}"
                if total_cost > 0:
                    order_summery += f"\nTotal Cost: {symbol}{total_cost}"
                    messagebox.showinfo(
                        "Order Placed, Thanks for free money",
                        order_summery)
                else:
                    messagebox.showerror("ERROR", "PLS ORDER AT LEAST ONE ITEM, U NO FOOD IF NO DO")
if __name__ == "__main__":
    root = tk.Tk()
    app = RestrauntOrderManagment(root)
